# Deploying amjad.auraplatform.org

The Strategic Member site lives at `amjad/website/` in the codebase. In production it
must resolve at `https://amjad.auraplatform.org/` — the directory is mounted as the
...
project whose publish/output directory is **`amjad/website/`**. No build step — plain
HTML/CSS/JS.
...
1. **Aura Meetings meeting type.** The booking CTA on the page links to:
   ...
   ```
   https://auraplatform.org/i/aura-platform-llc/meet/let-s-connect
   ```

   This meeting type exists in Aura Meetings (institution `aura-platform-llc`,
   slug `let-s-connect`). Confirm it resolves before DNS cutover.
...
```
dig +short amjad.auraplatform.org
curl -I https://amjad.auraplatform.org/
curl -s -o /dev/null -w "%{http_code}  /\n" https://amjad.auraplatform.org/
curl -s https://amjad.auraplatform.org/robots.txt
curl -s https://amjad.auraplatform.org/sitemap.xml
curl -I https://amjad.auraplatform.org/assets/social/og-default.png
```
...
`assets/social/og-default.png`; the metadata identifies Amjad as the person
represented by this estate.

The source profile PDF (`amjad/amjad.pdf`) sits **outside** the web root by design.
Do not move it into `amjad/website/` — it is reference material, not published
content.
